...
```jsonc
{
  ...
  "create": ".../internal/service/s3.resourceBucketCreate",   // SDKv2: handler func
  "read":   ".../internal/service/s3.resourceBucketRead",
  "update": ".../internal/service/s3.resourceBucketUpdate",
  "delete": ".../internal/service/s3.resourceBucketDelete",
...
// A Plugin Framework resource looks the same but the CRUD symbols are methods:
//   "create": ".../internal/service/appsync.(*apiResource).Create"
```

- The four `create`/`read`/`update`/`delete` values are full Go import-path
  symbols — SDKv2 handler funcs (`resourceBucketCreate`) or Plugin Framework
  CRUD methods (`(*apiResource).Create`). A slot is omitted when the provider
  has no own handler for it: `update` on an immutable resource, or a Framework
  CRUD method *promoted* from an embedded framework base (a no-op, e.g.
  `WithNoOpDelete` / `withNoOpUpdate[T]`). Promoted methods are detected by their
  `<autogenerated>` source location and skipped (they make no SDK calls); a
  method the resource declares itself is always emitted.
