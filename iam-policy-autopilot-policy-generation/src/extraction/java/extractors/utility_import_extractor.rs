...
/// (e.g. `"s3"`, `"dynamodb"`).
///
/// For wildcard imports (e.g. `import software.amazon.awssdk.enhanced.dynamodb.*`),
/// `class_name` is `"*"` — the caller must treat this as "package in scope" evidence
/// rather than a specific class import.
///
/// Returns `None` if the path does not match any known utility package prefix.
...
mod tests {
    ...
    // Wildcard utility imports are now stored with class_name = "*" so the matcher
    // can use them as "package in scope" evidence.
    #[case("software.amazon.awssdk.transfer.s3.*", Some(("s3", "*")))]
    #[case("software.amazon.awssdk.enhanced.dynamodb.*", Some(("dynamodb", "*")))]
