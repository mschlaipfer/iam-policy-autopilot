...
///
/// Retains only services that appear in `imported_services`.
/// Returns an empty list if `imported_services` is empty or if no candidates match;
/// callers must not emit an [`SdkMethodCall`] in that case.
///
...
pub(crate) fn resolve_services_by_type_name(
    ...
    if let Some(svc) = service {
        ...
        //
        // Guard: only attempt java_service_name matching when the file contains at
        // least one AWS wildcard import (`software.amazon.awssdk.services.<name>.*`).
        // `file_imports` holds only AWS imports (non-AWS imports are discarded by the
        // extractor), so checking for a trailing `.*` is sufficient.
        // Without this guard a non-AWS wildcard import (e.g. `import com.example.*;`)
        // combined with a coincidentally-named type (e.g. `S3Client`) and a matching
        // method name would produce a false-positive attribution.
        let has_aws_wildcard = file_imports.iter().any(|imp| imp.expr.ends_with(".*"));
        if !has_aws_wildcard {
            return Vec::new();
        }
        ...
        all_services
            ...
            .filter(|svc| {
                service_index
                    ...
                    .get(svc)
                    .map(|def| {
                        let base = def.metadata.java_service_name();
                        type_suffixes
                            .iter()
...
mod tests {
    ...
    #[case(vec!["s3", "dynamodb"], vec![], vec![], "empty imports filter everything out")]
