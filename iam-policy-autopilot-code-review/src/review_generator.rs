...
pub struct ReviewInput {
    ...
    pub suggest_policy_changes: bool,
    /// Glob patterns to filter which source files are analyzed.
    pub filters: Vec<String>,
}

/// Full review output ready for the GitHub Create Review API.
#[derive(Debug, Clone, serde::Serialize)]
pub struct ReviewOutput {
    /// Summary body for the review (top-level comment).
    pub body: String,
...
pub async fn generate_review(input: ReviewInput) -> Result<ReviewOutput> {
    ...
    let mut supported_files: Vec<(String, Language)> = all_files
        .into_iter()
    ...
    // ── Collect head results for policy suggestion ─────────────────────────────
    let head_results: Vec<&GeneratePoliciesResult> = result_map
        .iter()
        .filter(|((_, is_head), _)| *is_head)
        .map(|(_, result)| result)
        .collect();
...
/// Build the summary body for the review.
...
fn build_summary(
    ...
    if input.include_tool_link {
        summary.push_str(
            "\n<sub>Generated by [IAM Policy Autopilot](https://github.com/awslabs/iam-policy-autopilot)</sub>\n",
        );
    }
...
/// Build suggested policies for missing and conditional/resource-restricted actions.
///
/// Produces up to two collapsible sections:
/// 1. Actions that are completely missing from the policy (not allowed, not denied)
/// 2. Actions that are allowed but only conditionally or with restricted resources
#[allow(clippy::format_push_string)]
fn build_policy_suggestion(
    ...
    use iam_policy_autopilot_policy_generation::{
        IamPolicy, PolicyGenerationEngine, PolicyWithMetadata,
    };

    let checker = policy_checker?;

    let mut missing_policies: Vec<PolicyWithMetadata> = Vec::new();
    let mut constrained_policies: Vec<PolicyWithMetadata> = Vec::new();

    for result in head_results {
        for policy_meta in &result.policies {
            let mut missing_stmts = Vec::new();
            let mut constrained_stmts = Vec::new();

            for stmt in &policy_meta.policy.statements {
                let all_missing = stmt
                    .action
                    .iter()
                    .all(|action| checker.check_action(action).is_missing());
                if all_missing {
                    missing_stmts.push(stmt.clone());
                    continue;
                }
                let any_constrained = stmt.action.iter().any(|action| {
                    let status = checker.check_action(action);
                    status.allowed
                        && (!status.conditions.is_empty()
                            || !status.restricted_resources.is_empty())
                });
                if any_constrained {
                    constrained_stmts.push(stmt.clone());
                }
            }
            ...
            if !missing_stmts.is_empty() {
                ...
                missing_policies.push(PolicyWithMetadata {
                    policy,
                    policy_type: policy_meta.policy_type,
                });
            }
            if !constrained_stmts.is_empty() {
                let mut policy = IamPolicy::new();
                for stmt in constrained_stmts {
                    policy.add_statement(stmt);
                }
                constrained_policies.push(PolicyWithMetadata {
                    policy,
                    policy_type: policy_meta.policy_type,
                });
            }
    ...
    let engine = PolicyGenerationEngine::new("aws", "*", "*");
    let mut output = String::new();

    if !missing_policies.is_empty() {
        if let Ok(merged) = engine.merge_policies(&missing_policies) {
            if let Some(first) = merged.first() {
                if let Ok(json) = serde_json::to_string_pretty(&first.policy) {
                    output.push_str(
                        "\n<details><summary>Suggested policy for missing actions</summary>\n\n",
                    );
                    output.push_str(
                        "These actions are not covered by any Allow statement and are not \
                         explicitly denied.\n\n",
                    );
    ...
    if !constrained_policies.is_empty() {
        if let Ok(merged) = engine.merge_policies(&constrained_policies) {
            if let Some(first) = merged.first() {
                if let Ok(json) = serde_json::to_string_pretty(&first.policy) {
                    output.push_str(
                        "\n<details><summary>Suggested policy for conditional/resource-restricted actions</summary>\n\n",
                    );
                    output.push_str(
                        "These actions are currently allowed only with conditions or restricted \
                         resources. The policy below would grant them unconditionally — review \
                         whether widening access is appropriate.\n\n",
                    );
                    output.push_str("```json\n");
                    output.push_str(&json);
                    output.push_str("\n```\n\n</details>\n");
